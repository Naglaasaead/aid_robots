...
class _ChatScreenState extends State<ChatScreen3> {
  ...
  Future<void> sendMessage({String? text, String? imageUrl}) async {
    DocumentReference docRef = _fireStore.collection('message3').doc();
    ...
    try {
      ...
      await _fireStore.collection('message3').doc(autoID).set(messageMap);
      setState(() {
        messageText = null;
        file = null;
      });
          ...
          PopupMenuButton<String>(
            onSelected: (String value) async {
              if (value == 'delete') {
                ...
                CollectionReference ref = FirebaseFirestore.instance.collection('message3');
                try {
...
class MessageStreamBuilder extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return StreamBuilder<QuerySnapshot>(
      stream: _fireStore.collection('message3').orderBy('time', descending: true).snapshots(),
