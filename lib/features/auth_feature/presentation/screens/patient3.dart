...
class Patient3 extends StatefulWidget {
  const Patient3({Key? key});

  @override
  State<Patient3> createState() => _PatientState();
}

class _PatientState extends State<Patient3> {
  ...
  Future<void> retrievePatients() async {
    try {
      CollectionReference patientsRef = FirebaseFirestore.instance.collection('docPatient2');
...
class _PatientState extends State<Patient3> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text('Patients of surgery ',style: TextStyle(color: Colors.black,fontWeight: FontWeight.bold,fontSize: 20),),
      ),
      floatingActionButton: FloatingActionButton(
        onPressed: () {
          showDialog(
